...
func PrintSystemQueryResponse(out io.Writer, resp *control.SystemQueryResp, opts ...PrintConfigOption) error {
	if resp == nil {
	...
	case len(resp.Members) == 0:
		fmt.Fprintln(out, "Query matches no members in system.")
...
func printSystemAction(out io.Writer, results system.MemberResults, absentHosts *hostlist.HostSet, absentRanks *system.RankSet) error {
	groups := make(system.RankGroups)
...
func PrintSystemStartResponse(out io.Writer, resp *control.SystemStartResp) error {
	return printSystemAction(out, resp.Results, &resp.AbsentHosts, &resp.AbsentRanks)
}
...
func PrintSystemStopResponse(out io.Writer, resp *control.SystemStopResp) error {
	return printSystemAction(out, resp.Results, &resp.AbsentHosts, &resp.AbsentRanks)
}
