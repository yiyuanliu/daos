...
dfs_sys_access(dfs_sys_t *dfs_sys, const char *path, int mask, int flags)
{
	...
	if (flags & AT_SYMLINK_NOFOLLOW) {
		...
		if (rc) {
			 ...
			 * Maybe just DEBUG */
			D_ERROR("dfs_lookup_rel() %s failed: (%d)\n",
				sys_path.name, rc);
		  ...
		  daos_size_t *size, int flags)
{
	...
	if (rc) {
		D_ERROR("dfs_listxattr() %s failed: (%d)\n",
			sys_path.name, rc);
		 ...
		 void *value, daos_size_t *size, int flags)
{
	...
	if (rc) {
		D_ERROR("dfs_getxattr() %s failed: (%d)\n", name, rc);
		D_GOTO(out_free_obj, rc);
	}
		 ...
		 const void *value, daos_size_t size, int set_flags, int flags)
{
	...
	if (rc) {
		D_ERROR("dfs_setxattr() %s failed: (%d)\n", name, rc);
		D_GOTO(out_free_obj, rc);
	}
		 ...
		 daos_size_t *size)
{
	...
	if (rc) {
		D_ERROR("dfs_get_symlink_value() %s failed: (%d)\n",
			sys_path.name, rc);
		D_GOTO(out_free_obj, rc);
	}
